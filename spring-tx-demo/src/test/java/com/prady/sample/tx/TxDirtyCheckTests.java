...
import com.prady.sample.tx.service.TxDirtyCheckService;

/**
 * @author Prady
 */
public class TxDirtyCheckTests extends BaseTests {

    @Autowired
    private TxDirtyCheckService txDirtyCheckService;
    @Autowired
    ...
    void entityDirtyCheckWithoutSaveNewTx() {
        ...
        txDirtyCheckService.entityDirtyCheck(customerIds.get(0), Boolean.FALSE, Boolean.TRUE);
    ...
    void entityDirtyCheckWithSaveNewTx() {
        ...
        Exception exception = Assertions.assertThrows(ObjectOptimisticLockingFailureException.class, () -> {
            txDirtyCheckService.entityDirtyCheck(customerIds.get(0), Boolean.TRUE, Boolean.TRUE);
        });
    ...
    void entityDirtyCheckWithoutSaveDefault() {
        List<Long> customerIds = dataStoreHelper.createCustomers(port, 1);
        txDirtyCheckService.entityDirtyCheck(customerIds.get(0), Boolean.FALSE, Boolean.FALSE);
        CustomerDTO customer = customerService.getCustomer(customerIds.get(0));
        Assertions.assertEquals("Updated First Name", customer.getFirstName());
        Assertions.assertEquals("Updated Last Name", customer.getLastName());
    }
    ...
    void entityDirtyCheckWithSaveDefault() {
        List<Long> customerIds = dataStoreHelper.createCustomers(port, 1);
        txDirtyCheckService.entityDirtyCheck(customerIds.get(0), Boolean.TRUE, Boolean.FALSE);
        CustomerDTO customer = customerService.getCustomer(customerIds.get(0));
        Assertions.assertEquals("Updated First Name", customer.getFirstName());
        Assertions.assertEquals("Updated Last Name", customer.getLastName());
    }
